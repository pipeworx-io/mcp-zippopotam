...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/lookup_zipcode \
  -H 'Content-Type: application/json' \
  -d '{"country":"us","zipcode":"90210"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/lookup_zipcode`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "zippopotam": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-zippopotam"]
    }
  }
}
```
